	...
	for _, f := range z.File {
		if f != nil {
			...
			if f.Name == "word/document.xml" {
				...
				if err := d.document.Decode(reader); nil != err {
					return nil, err
				}
				...
				header := new(Header)
				if err := header.Decode(reader); nil != err {
					return nil, err
				}
				if err := reader.Close(); err != nil {
					return nil, err
					...
					if zf.Name == "word/document.xml" {
						...
						if wzf != nil {
							if b, err := wordDocumentToXML(f.document); b != nil && err == nil {
								if _, err := wzf.Write([]byte("<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>")); nil != err {
									return err
								}
								if _, err := wzf.Write(b); nil != err {
									return err
								}
							}
						}
